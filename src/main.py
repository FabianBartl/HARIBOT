...
from nextcord import Member, User, Guild, Message, Interaction, SlashOption, File, Embed, Permissions, Role, Reaction, \
    VoiceState
...
async def on_message(message: Message):
    ...
    guild = message.guild
    LOG.LOGGER.debug(
        f"(msg sent) {channel.name} - {author.display_name}: {f'({len(attachments)} Attachments)' if len(attachments) > 0 else ''} '{content}'")
...
@bot.slash_command(name="test", description="Test some new stuff.",
...
@bot.slash_command(name="log", description="Manage logging files.",
                   default_member_permissions=Permissions(administrator=True))
async def sc_log(
        interaction: Interaction
        , action: int = SlashOption(required=True,
                                    choices={"backup": 0, "save": 1, "get": 2, "clear": 3, "reset": 4, "list": 5})
):
...
async def sc_serverInfo(interaction: Interaction):
    ...
    embed.add_field(name="Invite",
                    value=f"[open](https://discord.gg/GVs3hmCmmJ) or copy invite:\n```\nhttps://discord.gg/GVs3hmCmmJ\n```",
                    inline=False)
...
async def sc_botInfo(interaction: Interaction):
    ...
    try:
        ...
        contrib_dict = {contrib["login"]: {"profile": contrib["html_url"], "num": contrib["contributions"]} for contrib
                        in contrib_list}
        contrib_value = ", ".join(
            [f"[{contrib}]({contrib_dict[contrib]['profile']}): {contrib_dict[contrib]['num']}" for contrib in
             contrib_dict])
...
@bot.slash_command(name="auto-role", description="Manage auto roles.",
                   default_member_permissions=Permissions(administrator=True))
...
async def sc_reactionRole(
    ...
    await interaction.response.send_message(f"`reaction-role`: `{action=}`, `{messageID=}`, `{emoji=}`, `{role}`",
...
async def sc_whatIf(
    ...
    if num is None:
        num = random.randint(0, image_count)
    elif num > image_count:
        num = image_count
